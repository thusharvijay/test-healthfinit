...
export function useDashboardState({
  ...
  const loadUserData = useCallback(async () => {
    try {
      ...
      setTrial({
        left: dashboardStats.trial_cases_left || 7,  // ✅ Use trial_cases_left from backend
        used: 7 - (dashboardStats.trial_cases_left || 7),
        seenWelcome: loadLS(LS_KEYS.TRIAL(currentUser))?.seenWelcome || false,
      });
      if (!hasSeenWelcome && dashboardStats.trial_cases_left > 0) {
        setShowTrialModal(true);
      }
      setWallet(dashboardStats.wallet_balance);
      ...
      try {
        ...
        setWallet(walletInfo.wallet_balance);
  ...
  const addWalletBalance = async (amount) => {
    try {
      console.log('=== ADD WALLET BALANCE DEBUG ===');
      
      // Check localStorage
      const tokenFromStorage = localStorage.getItem('access_token');
      console.log('1. Token in localStorage:', tokenFromStorage ? 'EXISTS' : 'NULL');
      console.log('   Token length:', tokenFromStorage?.length || 0);
      console.log('   First 20 chars:', tokenFromStorage?.substring(0, 20) || 'N/A');
      
      // Set token
      if (tokenFromStorage) {
        apiService.setToken(tokenFromStorage);
        console.log('2. Token set in apiService');
      } else {
        console.error('2. NO TOKEN IN LOCALSTORAGE!');
      }
      
      // Verify token is in apiService
      const tokenFromService = apiService.getToken();
      console.log('3. Token from apiService:', tokenFromService ? 'EXISTS' : 'NULL');
      console.log('   Match:', tokenFromStorage === tokenFromService);
      
      console.log('4. Making API call...');
      const response = await apiService.rechargeWallet(amount, 'demo');
      
      console.log('5. API call successful:', response);
      //setWallet(response.new_balance);
      //setSuccess(response.message || `₹${amount} added successfully`);

      notificationService.addNotification(
        'success',
        'Wallet Recharged',
        `₹${amount} has been added to your wallet. New balance: ₹${response.new_balance}`
      );
